...
void *readThread(void *arg){
	...
	while(1){
		...
		if(cHeader.msgType == (unsigned char)HLLO){
			...
			//printf(" READ THREAD - Connection %d : Generated Status response \n",argForThread->ID);
			/*
			len = -1;
			memcpy(&len,statusResponseMsg.msgData,4);
			...
			*/
			...
			Construct the status response' data part. The signal at line 191 will send the data.
			For the data part you need to place all the neighboring port number and host name in a variable as in the spec.
			All the neighbouring port number and hostname will be in connections[i] data structure. Loop through them and send it.
			You dont need any other mutex. 
			...
			*/
			...
			Parse the response you sent above and store it in a local variable
