...
void *timerThread(void *s){
	...
	while(1){
		...
		for(unsigned int i=0;i<noOfConnections;i++){
			if(connections[i].timestamp - (tv.tv_sec + (tv.tv_usec/1000000.0)) >= (60)){
				
				printf(" READ THREAD - Connection %d : Timestamp %f ,current Time : %f\n",argForThread->ID,connections[argForThread->ID].timestamp,(tv.tv_sec + (tv.tv_usec/1000000.0)));
...
void sig_usr(int signum){
	printf(" USER INPUT THREAD : Signal received %d \n",signum);
}
...
int main(unsigned int argc,char **argv){
	...
	while(1){
		...
		printf(" Port : %d \t Location : %d \t HomeDir : %s MsgLifetime : %d JoinTimeOut : %d\n",port,location,homeDir,msgLifeTime,joinTimeOut);
		...
		if(!iAmABeacon){
			if(!(initNeighborListFileAvailable)){    // && noOfNeighbors >= minNeighbors
				...
				argForClientThread[0].ID = 0;
				if(pthread_create(&client[0],NULL,&clientThread,(void *)&argForClientThread[0]) < 0){
					printf("\n\tPthread creation failed");
					return 0;
				}
